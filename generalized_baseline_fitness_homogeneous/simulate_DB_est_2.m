function result = simulate_DB_est_2(uff, ufn, unn, graph, alpha, iterate_time, N, p_ini, b)
%   Date: 2019.07.25
%   This code is modifed based on "simulated_DB_est.m"
%   This code has different fitness calculation of neighbors, for
%   neirghbors having a exact heighbor which is the center user.
...
%     end
    ...
    function fit_result = fitness_calculate_est(index, sf_ratio, center_str)
        %calculate fitness
        neigh = degree_table(index);
        if center_str == 1
            if action_table(index)==1
                fit = (neigh - 1) * sf_ratio * uff + (neigh - 1) * (1 - sf_ratio) * ufn + uff; 
            else
                fit = (neigh - 1) * sf_ratio * ufn + (neigh - 1) * (1 - sf_ratio) * unn + ufn;
            end
        else
            if action_table(index)==1
                fit = (neigh - 1) * sf_ratio * uff + (neigh - 1) * (1 - sf_ratio) * ufn + ufn; 
            else
                fit = (neigh - 1) * sf_ratio * ufn + (neigh - 1) * (1 - sf_ratio) * unn + unn;
            end
        end
            ...
            if action_table(i) == 1
                for j = 1:friend_number
                    if action_table(friend_list(j)) == 1
                        fit_f = fit_f + fitness_calculate_est(friend_list(j), sf_ratio, 1);
                    else
                        fit_n = fit_n + fitness_calculate_est(friend_list(j), sf_ratio, 1);
                    end
                end
                ...
                for j = 1:friend_number
                    if action_table(friend_list(j)) == 1
                        fit_f = fit_f + fitness_calculate_est(friend_list(j), sf_ratio, 0);
                    else
                        fit_n = fit_n + fitness_calculate_est(friend_list(j), sf_ratio, 0);
                    end
                end
