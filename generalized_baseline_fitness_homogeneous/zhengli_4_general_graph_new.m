% after talk with Dr. Zhao, we decide to use the estimated way to calculate
% 2019.07.24
syms alpha b k uff ufn unn pf kf gamma_f gamma_n kf_3 kf_2 kf_1 m Phi_f Phi_n
...
fit_ff = (1 - alpha) * b + alpha*( (m - 1) * kf/k * uff + (m - 1) * (k - kf)/k * ufn + uff );
fit_nf = (1 - alpha) * b + alpha*( (m - 1) * kf/k * ufn + (m - 1) * (k - kf)/k * unn + ufn );
fit_fn = (1 - alpha) * b + alpha*( (m - 1) * kf/k * uff + (m - 1) * (k - kf)/k * ufn + ufn );
fit_nn = (1 - alpha) * b + alpha*( (m - 1) * kf/k * ufn + (m - 1) * (k - kf)/k * unn + unn );
...
num_ap_1 = B*C - A*D;
num_ap_1 = collect(num_ap_1, [b, k, m, pf, kf]);
...
num_ap_2 = collect(num_ap_2, [b, k, m, pf, kf]);
...
% f1 = k*b*k*pf*(gamma_f2 - b) - b*(gamma_f2 - b)*(k*pf + k*(k - 1)*pf^2) - b*(gamma_n2 - b)*k*(k - 1)*pf*(1 - pf);
% f1 = expand(f1);
% f2 = k*b*k*(1 - pf)*(gamma_n1 - b) - b*(gamma_f1 - b)*k*(k - 1)*pf*(1 - pf) - b*(gamma_n1 - b)*(k*(1 - pf) + k*(k - 1)*(1 - pf)^2);
% f2 = expand(f2);
% result = (1 - pf)*f2 - pf*f1
